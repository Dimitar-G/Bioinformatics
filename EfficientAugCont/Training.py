...
from Transforms import transformAugment, transformAugmentVal
...
if __name__ == '__main__':
    ...
    # Training Data
    dataset = ALLDataset(root='../Data/training_data', transform=transformAugment)
    dataloader = DataLoader(dataset, batch_size=64, shuffle=True, num_workers=2)
    print('Training dataset loaded.')

    # Validation Data
    validation_dataset = ALLDataset(root='../Data/validation_data', transform=transformAugmentVal)
    ...
    model.load_state_dict(torch.load('./models/efficient_aug_1091.pt'))
    ...
    for epoch in range(num_epochs):
        ...
        with open('training_info.csv', 'a') as file:
            file.write(f'{epoch+1097},{training_loss},{val_loss},{val_acc}\n')
            file.flush()

        if epoch % 10 == 0:
            torch.save(model.state_dict(), f"./models/efficient_aug_{epoch+1100}.pt")

    torch.save(model.state_dict(), f"./models/efficient_aug_{num_epochs+1100}.pt")
